...
def render_report(metrics: MetricsReport) -> str:
    """Render a full 8-section lab report from the metrics data."""
    scenario_rows = "\n".join(
    ...
    resume_evidence = (
        "SQLite checkpointer used with WAL journal mode. One thread_id per run. "
        "After all scenarios completed, `graph.get_state()` was called for the first "
        "error-route scenario and the full persisted state was recovered, confirming "
        "crash-resume capability. State history depth also verified via "
        "`graph.get_state_history()`."
        if metrics.resume_success
        else (
            "MemorySaver checkpointer used (in-process, not persistent). "
            "For SQLite crash-resume evidence, set `checkpointer: sqlite` in lab.yaml."
        )
    )
...
**Summary**: {metrics.total_scenarios} scenarios — {success_pct} success rate —
avg {avg_nodes} nodes visited — {metrics.total_retries} total retries —
{metrics.total_interrupts} HITL interrupts — resume_success={metrics.resume_success}
...
def render_report_stub(metrics: MetricsReport) -> str:
    """Alias kept for backwards compatibility — delegates to render_report."""
